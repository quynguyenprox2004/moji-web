 ...
 * Hiểu đơn giản: Khi ứng dụng bắt đầu chạy lên, code sẽ chạy vào main.jsx đầu tiên, từ bên đó chúng ta sẽ gọi hàm injectStore ngay lập tức để gán biến mainStore vào biến axiosReduxStore cục bộ trong file này
 * https://redux.js.org/faq/code-structure#how-can-i-use-the-redux-store-in-non-component-files
 */
let axiosReduxStore
export const injectStore = mainStore => { axiosReduxStore = mainStore }

// Khởi tạo một đối tượng Axios (authorizedAxiosInstance) mục đích để custom và cấu hình cung cho dự án.
let authorizedAxiosInstance = axios.create()
// Thời gian chờ tối đa của 1 request: để 10 phút
authorizedAxiosInstance.defaults.timeout = 1000 * 60 * 10
// withCredentials: Sẽ cho phép axios tự động gửi cookie trong mỗi request kên BE (phục vụ việc chúng ta sẽ lưu JWT tokens (refresh & access) vào trong httpOnly Cookie của trình duyệt)
...
// Mục đích tạo Promise này để khi nào gọi api refresh_token xong xuôi thì mới retry lại nhiều api bị lỗi tước đó
...
authorizedAxiosInstance.interceptors.response.use((response) => {
  ...
  // Trường hợp 2: Nếu như nhận mã 410 từ BE, thì sẽ gọi api refresh token để làm mới accessToken
  //Đầu tiên lấy được các request API đang bị lỗi trong qua error.config
  const originalRequests = error.config
  // console.log('originalRequests: ', originalRequests)
  if (error.response.status === 410 && !originalRequests._retry) { // có thể không dùng _retry cũng được vì cái refreshTokenPromise để check là đủ rồi.
    ...
    if (!refreshTokenPromise) {
      refreshTokenPromise = refreshTokenAPI()
        ...
        .catch((_error) => {
          // Nếu nhận bất kỳ lỗi nào từ api refresh token thì cứ cho logout luôn
          axiosReduxStore.dispatch(logoutUserAPI(false))
